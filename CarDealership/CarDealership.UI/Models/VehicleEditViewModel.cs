...
{
    // model needed for Admin/VehicleEdit view
    // mostly the same as VehicleAdd, but will not require a new image, and has "is featured" checkbox
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (ImageUpload != null)
            {
                if (ImageUpload.ContentLength <= 0)
                    errors.Add(new ValidationResult("Image file cannot be empty."));
                else
                {
